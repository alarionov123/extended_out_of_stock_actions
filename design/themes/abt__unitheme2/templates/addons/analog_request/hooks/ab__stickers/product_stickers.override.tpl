{* Removes sticker from the out of stock product with Out of stock action set as Request analog *}
            ...
            <div class="ab-stickers-container ab-stickers-container__{$position@key}{$addons.ab__stickers.output_position} {$addons.ab__stickers.{"`$position@key``$addons.ab__stickers.output_position`_output_type"}}-filling">
                {foreach $position as $sticker}
                    ...
                    {if $pos_counter < $addons.ab__stickers.{"`$position@key``$addons.ab__stickers.output_position`_max_count"}}
                        ...
                        <div class="ab-sticker-{$view_type|default:"full_size"}{if $sticker.style == $graphic_style} small-image-size-{$sticker.appearance.small_size_image_size} full-image-size-{$sticker.appearance.full_size_image_size}{/if}"
                             data-ab-sticker-id="{$sticker@key}"{if $sticker.placeholders} data-placeholders='{$sticker.placeholders}'{/if}></div>
                    {/if}
                {/foreach}
            </div>
        {/foreach}
