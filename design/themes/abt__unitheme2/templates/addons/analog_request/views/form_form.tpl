<div class="hidden" id="request_analog_{$object_id}" title="{__("request_analog")}">
    <div class="analogues_form">
        <form action="{""|fn_url}" method="post" class="{if !$no_ajax}cm-ajax{/if} cm-form-dialog-closer" name="request_analogs_{$object_id}" id="form_form_{$object_id}">
            ...
            {assign var=items value=fn_extended_out_of_stock_actions_get_analogues($object_id)}
            {if $items}
                <div class="ty-analog-items">
                    {include file="addons/extended_out_of_stock_actions/views/product_scroller.tpl" items=$items block=fn_scroller_properties() hide_wishlist_button=true hide_compare_list_button=true}
                </div>
                <div class="ty-analogues-additional-txt">
                    <h3>{__("if_analogues_do_not_suit")}</h3>
                </div>
            {/if}
